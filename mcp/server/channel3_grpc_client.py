...
from channel3_tunnel import Channel3Tunnel
...
class GrpcChannel3Error(VsockError):
    pass


class GrpcChannel3Client:
    def __init__(
        self,
    ...
    def _load_grpc_modules(self) -> None:
        if grpc is None:
            raise GrpcChannel3Error(
                f"grpcio 未安装，无法启用 gRPC Channel 3: {_GRPC_IMPORT_ERROR}"
            )
        ...
        try:
            from generated import channel3_pb2, channel3_pb2_grpc
        except ImportError as exc:  # pragma: no cover - depends on runtime image
            raise GrpcChannel3Error(
                f"protobuf stubs 未安装，无法启用 gRPC Channel 3: {exc}"
            ) from exc

        self._pb2 = channel3_pb2
        self._pb2_grpc = channel3_pb2_grpc
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self._stub = self._pb2_grpc.Channel3ServiceStub(self._channel)
        except Exception as exc:
            self.close()
            raise GrpcChannel3Error(f"连接 gRPC Channel 3 失败: {exc}") from exc
    ...
    def begin_task(self, session_id: str, root_description: str) -> str:
        ...
        if not response.task_id:
            raise GrpcChannel3Error("BeginTask 响应缺少 task_id")
    ...
    def end_task(self, status: str = "success") -> None:
        ...
        if not response.success:
            raise GrpcChannel3Error("EndTask 返回 success=false")
        self._task_id = ""
    ...
    def call_capability(self, capability: str, operation: str, params: JSON) -> JSON:
        ...
        try:
            ...
            raise GrpcChannel3Error(f"CallCapability result_json 非法: {exc}") from exc
