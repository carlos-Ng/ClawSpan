#include "channel3_grpc_server.h"
...
Channel3RpcService::Channel3RpcService(BeginTaskHandler  begin_handler,
...
grpc::Status Channel3RpcService::BeginTask(grpc::ServerContext*     /*context*/,
...
grpc::Status Channel3RpcService::EndTask(grpc::ServerContext*   /*context*/,
...
grpc::Status Channel3RpcService::CallCapability(grpc::ServerContext*         /*context*/,
...
Channel3GrpcServer::Channel3GrpcServer(BeginTaskHandler  begin_handler,
                                       EndTaskHandler    end_handler,
                                       CapabilityHandler capability_handler)
	: service_(std::move(begin_handler),
	           std::move(end_handler),
	           std::move(capability_handler))
{}

Channel3GrpcServer::~Channel3GrpcServer()
{
	stop();
}

Status Channel3GrpcServer::start(std::string_view listen_address)
{
	if (server_ != nullptr) {
		return Status(Status::INTERNAL_ERROR, "channel3 grpc server is already running");
	}
	...
	if (server_ == nullptr) {
		return Status(Status::IO_ERROR, "failed to start channel3 grpc server");
	}
	...
	LOG_INFO("channel3 grpc: listening on {}", listen_address);
	return Status::Ok();
}

void Channel3GrpcServer::stop()
{
	if (server_ == nullptr) {
		return;
	}
	...
	server_.reset();
	LOG_INFO("channel3 grpc: stopped");
}

bool Channel3GrpcServer::isRunning() const
{
